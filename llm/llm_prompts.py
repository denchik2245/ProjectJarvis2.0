...
10. Для intent "add_contact" объект "parameters" должен содержать:
    - "contact_name": строка (имя контакта),
    - "phone": строка (номер телефона),
    - Опционально "company": строка,
    - Опционально "birthday": строка в формате "DD.MM.YYYY" или "DD.MM".
11. Если запрос не соответствует ни одному известному шаблону, возвращай:
   {
     "intent": "unknown",
     "parameters": {}
   }
12. Если имя задано в склонённой форме (например, "Антону", "Антохи"), нормализуй его до именительного падежа (например, "Антон").
...
[Пример 13]
Пользователь: "Добавь контакт Глеб из ЧелГУ, день рождения 15 июля, номер +79126973674"
Ответ:
{
  "intent": "add_contact",
  "parameters": {
