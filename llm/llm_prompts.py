...
2. Поддерживаемые значения поля "intent":
   ...
   - "current_weather" — для получения текущей погоды в указанном городе.
   - "current_temperature" — для получения текущей температуры в указанном городе.
   - "week_forecast" — для получения прогноза погоды на следующую неделю в указанном городе.
...
18. Для intent "cancel_meeting" объект "parameters" должен содержать:
    - "contact_name": строка.
19. Для intent "current_weather" объект "parameters" должен содержать:
    - "city": строка (например, "Москва").
20. Для intent "current_temperature" объект "parameters" должен содержать:
    - "city": строка (например, "Сочи").
21. Для intent "week_forecast" объект "parameters" должен содержать:
    - "city": строка (например, "Екатеринбург").
22. Если запрос не соответствует ни одному известному шаблону, возвращай:
   {
     "intent": "unknown",
     "parameters": {}
   }
23. Если имя задано в склонённой форме, нормализуй его до именительного падеже.
...
[Пример 22]
Пользователь: "Какая будет погода в Сочи на следующей неделе?"
Ответ:
{
  "intent": "week_forecast",
  "parameters": {
    "city": "Сочи"
  }
}
